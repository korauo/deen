...
async function getHelpMenu({ client, guild }) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**About Me:**\nAssalamu Alaikum, I'm ${guild.members.me.displayName}\n > An Islamic Discord Bot on a mission to make the Quran and Hadith accessible for everyone. 
      \n**Invite Me:** [Here](${client.getInvite()}) - **Support Server:** [Join](${SUPPORT_SERVER}) `,
    );
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `page ${index + 1} of ${
          arrSplitted.length
        } | Type ${prefix}help <command> for specific info`,
      });
    arrEmbeds.push(embed);
  });
